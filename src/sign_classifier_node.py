#!/usr/bin/env python3
"""
Traffic Sign Classifier Node
=============================
Downstream ROS node that classifies the *type* of traffic signs detected by
the YOLOv10 detector node.

Architecture
------------
The detector node publishes raw bounding boxes for every object class including
``traffic sign``.  This node subscribes to those detections **and** the
corresponding decoded image, filters for traffic-sign boxes, crops the ROI, and
runs a fine-tuned EfficientNet B0 classifier to identify the sign category.

When the predicted type is ``speed_limit``, the bounding box and image crop are
forwarded to the SpeedLimitOCRNode via the ``/perception/traffic_signs`` topic
so the OCR node can read the actual numeric value.

Subscriptions
-------------
  ~detection_topic  (yolov10_ros/DetectionArray)  default: /yolov10/detections
      Bounding boxes from the detector node.
  ~image_topic      (sensor_msgs/Image)            default: /yolov10/image_raw
      Decoded source frame republished by the detector node.

  Both topics are time-synchronised via message_filters.ApproximateTimeSynchronizer
  (slop = 0.1 s) so crops always correspond to the correct frame.

Publications
------------
  ~output_topic  (yolov10_ros/ClassifiedSignArray)  default: /perception/traffic_signs
      One ClassifiedSign per confirmed sign, carrying the predicted type string,
      its softmax confidence, and the original bounding box coordinates.

ROS Parameters
--------------
  ~weights               (str)   Path to EfficientNet B0 .pth checkpoint.
  ~confidence_threshold  (float) Minimum classifier softmax confidence to publish.
                                 Default: 0.5
  ~device                (str)   CUDA device index ("0") or "cpu".  Default: "0"
  ~target_class          (str)   Detector class name to filter for.
                                 Default: "traffic sign"
  ~roi_padding           (int)   Extra pixels added around each bounding box
                                 before cropping.  Default: 5
  ~class_names           (list)  Override the default class mapping via YAML
                                 (list in alphabetical training order).

Class Mapping (default — 15 US traffic sign categories)
--------------------------------------------------------
   0  detour               — Temporary detour sign
   1  do_not_enter         — Wrong-way / do not enter
   2  go_straight_only     — Mandatory straight ahead
   3  no_left_turn         — No left turn prohibition
   4  no_right_turn        — No right turn prohibition
   5  no_straight          — No straight ahead
   6  no_u_turn            — No U-turn
   7  pedestrian_crossing  — Pedestrian crosswalk warning
   8  railroad_crossing    — Railroad grade crossing warning
   9  roadwork             — Construction / road work ahead
  10  speed_limit          — Speed limit sign (value read by OCR node)
  11  stop                 — Stop sign
  12  turn_left_only       — Mandatory left turn
  13  turn_right_only      — Mandatory right turn
  14  yield                — Yield / give way

Notes
-----
- The checkpoint key is ``model_state_dict`` (training saves a dict with
  epoch, optimizer state, etc.).
- ImageNet mean/std normalisation is applied before inference.
- ``speed_limit`` detections are published normally here; the SpeedLimitOCRNode
  listens on the output topic and runs OCR specifically on those signs.
"""

import rospy
import cv2
import torch
import torch.nn.functional as F
import numpy as np
import message_filters
from sensor_msgs.msg import Image
from yolov10_ros.image_utils import imgmsg_to_cv2, crop_roi


# ── EfficientNet B0 preprocessing constants ────────────────────────────────────
# Must match the values used during training exactly.
_INPUT_SIZE = 224
_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)   # ImageNet RGB mean
_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32)   # ImageNet RGB std

# ── Default class mapping ─────────────────────────────────────────────────────
# Indices are assigned by ImageFolder's alphabetical directory sort at training
# time.  If the training dataset is reorganised, this dict must be updated.
_DEFAULT_CLASSES = {
    0:  "detour",
    1:  "do_not_enter",
    2:  "go_straight_only",
    3:  "no_left_turn",
    4:  "no_right_turn",
    5:  "no_straight",
    6:  "no_u_turn",
    7:  "pedestrian_crossing",
    8:  "railroad_crossing",
    9:  "roadwork",
    10: "speed_limit",        # Numeric value extracted by SpeedLimitOCRNode
    11: "stop",
    12: "turn_left_only",
    13: "turn_right_only",
    14: "yield",
}


class SignClassifierNode:
    """EfficientNet B0 traffic-sign type classifier wrapped as a ROS node.

    Processes each DetectionArray message, crops every ``traffic sign`` ROI
    from the synchronised image, runs the classifier, and publishes a
    ClassifiedSignArray containing only high-confidence predictions.
    """

    def __init__(self):
        # ── ROS parameters ────────────────────────────────────────────────────
        # Drop predictions whose softmax probability is below this threshold.
        self.conf_thres = rospy.get_param("~confidence_threshold", 0.5)
        # Device string: digit → CUDA device, otherwise passed to torch.device.
        self.device = str(rospy.get_param("~device", "0"))
        # Only boxes whose class_name matches this string are passed to the
        # classifier.  Defaults to the BDD100K class name for traffic signs.
        self.target_class = rospy.get_param("~target_class", "traffic sign")
        # Pixels added on all sides of the bounding box before cropping.
        self.roi_padding = rospy.get_param("~roi_padding", 5)
        weights = rospy.get_param("~weights", "")

        # ── Class mapping ─────────────────────────────────────────────────────
        # Allow overriding via YAML list so the node survives retraining with
        # a different set of sign categories.
        class_list = rospy.get_param("~class_names", None)
        if class_list:
            self.class_map = {i: name for i, name in enumerate(class_list)}
        else:
            self.class_map = _DEFAULT_CLASSES

        # ── Model loading ─────────────────────────────────────────────────────
        self.torch_device = torch.device(
            "cuda:{}".format(self.device) if self.device.isdigit() else self.device
        )
        from torchvision.models import efficientnet_b0
        # Build the architecture with a random head, resize the head to the
        # correct output dimension, then overwrite weights from the checkpoint.
        self.model = efficientnet_b0(weights=None)
        num_classes = len(self.class_map)
        # EfficientNet B0 final feature dimension is 1280.
        self.model.classifier[1] = torch.nn.Linear(
            self.model.classifier[1].in_features, num_classes
        )
        # Checkpoint format: {"epoch": int, "model_state_dict": OrderedDict, ...}
        ckpt = torch.load(weights, map_location=self.torch_device)
        self.model.load_state_dict(ckpt["model_state_dict"])
        self.model.to(self.torch_device)
        self.model.eval()
        rospy.loginfo("Sign classifier loaded on %s (%d classes)",
                      self.torch_device, num_classes)

        # ── Lazy message imports ──────────────────────────────────────────────
        from yolov10_ros.msg import DetectionArray, ClassifiedSign, ClassifiedSignArray
        self.ClassifiedSign = ClassifiedSign
        self.ClassifiedSignArray = ClassifiedSignArray

        # ── Synchronized subscribers ──────────────────────────────────────────
        # Pair detections and image on header timestamp (±0.1 s tolerance).
        det_topic = rospy.get_param("~detection_topic", "/yolov10/detections")
        img_topic = rospy.get_param("~image_topic", "/yolov10/image_raw")

        det_sub = message_filters.Subscriber(det_topic, DetectionArray)
        img_sub = message_filters.Subscriber(img_topic, Image)

        sync = message_filters.ApproximateTimeSynchronizer(
            [det_sub, img_sub], queue_size=10, slop=0.1
        )
        sync.registerCallback(self.callback)

        # ── Publisher ─────────────────────────────────────────────────────────
        output_topic = rospy.get_param("~output_topic", "/perception/traffic_signs")
        self.pub = rospy.Publisher(output_topic, ClassifiedSignArray, queue_size=10)

        rospy.loginfo("Sign classifier node ready — filtering for '%s'", self.target_class)

    def _preprocess(self, crop):
        """Resize a BGR crop to the EfficientNet input format.

        Resizes to 224 × 224, converts BGR → RGB, scales to [0, 1], and applies
        ImageNet mean/std normalisation.  Returns a (1, 3, 224, 224) float32
        tensor on the correct device.

        Args:
            crop: H×W×3 BGR numpy array (arbitrary size).

        Returns:
            torch.Tensor of shape (1, 3, 224, 224) on self.torch_device.
        """
        img = cv2.resize(crop, (_INPUT_SIZE, _INPUT_SIZE))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
        img = (img - _MEAN) / _STD
        # HWC → CHW → add batch dimension
        tensor = torch.from_numpy(img.transpose(2, 0, 1)).unsqueeze(0)
        return tensor.to(self.torch_device)

    @torch.no_grad()
    def callback(self, det_msg, img_msg):
        """Handle a synchronised (DetectionArray, Image) pair.

        Filters detections for target_class, crops each ROI, classifies it,
        and publishes a ClassifiedSignArray if any high-confidence results
        are found.  ``speed_limit`` signs are included in the published array
        and picked up by the SpeedLimitOCRNode.

        Args:
            det_msg: yolov10_ros/DetectionArray — bounding boxes from detector.
            img_msg: sensor_msgs/Image — source frame matching det_msg timestamp.
        """
        # Fast-path: skip frames with no traffic-sign detections.
        relevant = [d for d in det_msg.detections if d.class_name == self.target_class]
        if not relevant:
            return

        image = imgmsg_to_cv2(img_msg)

        out_msg = self.ClassifiedSignArray()
        out_msg.header = det_msg.header

        for det in relevant:
            # Expand the box slightly to avoid clipping sign edges.
            crop = crop_roi(image, det.xmin, det.ymin, det.xmax, det.ymax,
                            padding=self.roi_padding)
            if crop.size == 0:
                continue

            tensor = self._preprocess(crop)
            logits = self.model(tensor)
            probs = F.softmax(logits, dim=1)
            conf, idx = probs.max(dim=1)
            conf = float(conf)
            idx = int(idx)

            if conf < self.conf_thres:
                continue

            cs = self.ClassifiedSign()
            cs.sign_type = self.class_map.get(idx, "unknown")
            cs.type_confidence = conf
            cs.xmin = det.xmin
            cs.ymin = det.ymin
            cs.xmax = det.xmax
            cs.ymax = det.ymax
            out_msg.signs.append(cs)

        if out_msg.signs:
            self.pub.publish(out_msg)


if __name__ == "__main__":
    rospy.init_node("sign_classifier", anonymous=True)
    node = SignClassifierNode()
    rospy.spin()
